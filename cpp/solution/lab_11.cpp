...
cv::Mat performSegmentation(const cv::Mat& input_image, int& threshold_value, bool use_otsu)
{
  ...
  cv::Mat scaled_input;
  input_image.convertTo(scaled_input, CV_16UC1, scale);
  ...
  const double scaled_threshold = cv::threshold(scaled_input, segmented_image, threshold_value * scale, 255, thresh_type);
  threshold_value = static_cast<int>(scaled_threshold / scale);
